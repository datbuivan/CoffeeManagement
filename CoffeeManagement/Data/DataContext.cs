...
namespace CoffeeManagement.Data
{
    public class DataContext : IdentityDbContext<ApplicationUser, ApplicationRole, string>
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            // --- CẤU HÌNH CHO BẢNG RECIPE ---
            builder.Entity<Recipe>(builder =>
            {

                // Cấu hình mối quan hệ: Một Recipe chỉ thuộc về một ProductSize
                builder.HasOne(r => r.ProductSize)
                       .WithMany(ps => ps.Recipes) // Một ProductSize có nhiều Recipe
                       .HasForeignKey(r => r.ProductSizeId); // Khóa ngoại là ProductSizeId

                // Cấu hình mối quan hệ: Một Recipe chỉ chứa một Ingredient
                builder.HasOne(r => r.Ingredient)
                       .WithMany(i => i.Recipes) // Một Ingredient có thể nằm trong nhiều Recipe
                       .HasForeignKey(r => r.IngredientId); // Khóa ngoại là IngredientId
            });

            builder.Entity<Shift>(builder =>
            {
                builder.Property(s => s.Name).IsRequired().HasMaxLength(100);
            });

            builder.Entity<StaffShift>(builder =>
            {
                builder.ToTable("EmployeeShifts");

                // Mối quan hệ với User/Employee
                builder.HasOne(es => es.Staff)
                       .WithMany() // Giả sử User không cần danh sách EmployeeShifts
                       .HasForeignKey(es => es.StaffId);

                // Mối quan hệ với Shift
                builder.HasOne(es => es.Shift)
                       .WithMany(s => s.EmployeeShifts)
                       .HasForeignKey(es => es.ShiftId);

                // Tạo một index để đảm bảo một nhân viên chỉ có một ca trong một ngày
                builder.HasIndex(es => new { es.StaffId, es.WorkDate }).IsUnique();
            });
